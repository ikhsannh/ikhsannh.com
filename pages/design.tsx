  ...
  return (
    <div className='container mx-auto xl:px-[2rem] md:px-[0rem] sm:px-[1rem] px-[1rem]'>
      {posts.map((post, index) => (
        <Link href={'/writing/design/' + post.slug} passHref key={index}>
...
export const getStaticProps = async () => {
  const files = fs.readdirSync(path.join('posts/design'))

  const posts = files.map(filename => {
    const markdownWithMeta = fs.readFileSync(path.join('posts/design', filename), 'utf-8')
